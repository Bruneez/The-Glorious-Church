...
export default function TaskForm({
  ...
  const handleAssigneeChange = (event) => {
    ...
    const selectedOption = assigneeOptions.find((option) => option.value === selectedId);

    setFormData((prev) => ({
      ...prev,
      assignedUserId: selectedId,
      assignedUserName: selectedOption?.name || '',
      assignedUserRole: selectedOption?.role || '',
    }));
  };
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-3">
        <Input
          ...
          placeholder="e.g. Prepare Sunday service slides"
          required
        />
