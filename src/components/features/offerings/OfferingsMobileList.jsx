...
export default function OfferingsMobileList({
  offerings = [],
  onView,
  onEdit,
  onDelete,
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <h3 className="text-sm font-bold text-white">
                {offering.serviceDate ? formatDate(offering.serviceDate) : '—'}
              </h3>
              <p className="text-[11px] text-slate-500 mt-0.5">
                Created {formatCreatedAt(offering.createdAt)}
              </p>
            </div>
            <p className="text-sm font-bold text-emerald-400">
              {formatCurrencySimple(offering.totalAmount)}
            </p>
          </div>

          <p className="text-[11px] text-slate-500">
            Recorded by:{' '}
            <span className="text-slate-300 font-medium">{offering.recordedBy || '—'}</span>
          </p>
